...
export function useAvalon(roomId: string | undefined, room: Room | null, players: Player[]) {
  ...
  const writeState = useCallback(
    async (patch: Partial<AvalonGameState>) => {
      if (!roomId) return;
      await storage.updateRoomGameState(roomId, patch as never);
    },
    [roomId]
  );
  ...
  const assignRoles = useCallback(async () => {
    ...
    await storage.updatePlayersGameDataBatch(roomId, roleUpdates);
    ...
    await storage.updateRoomStatus(roomId, 'night');
  ...
  const beginTeamBuild = useCallback(async () => {
    ...
    await storage.updateRoomStatus(roomId, 'day');
  }, [roomId, writeState]);
  ...
  const ackRole = useCallback(
    async (playerId: string) => {
      ...
      await storage.updateRoomGameState(roomId, payload as never);
    },
    [roomId]
  );
  ...
  const submitTeam = useCallback(async () => {
    ...
    await storage.updateRoomStatus(roomId, 'voting');
  }, [roomId, writeState]);
  ...
  const proceedAfterTeamVoteResult = useCallback(async () => {
    ...
    if (state.lastTeamVoteResult === 'approved') {
      ...
      await storage.updatePlayersGameDataBatch(
        roomId,
        gamePlayers.map((p) => ({
          playerId: p.id,
          data: { questCard: null },
  ...
  const playQuestCard = useCallback(
    async (playerId: string, card: QuestCard) => {
      ...
      await storage.updatePlayerGameData(roomId, playerId, { questCard: card });
    },
    [roomId]
  );
