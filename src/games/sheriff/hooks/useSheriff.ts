...
export function useSheriff(
  ...
  const confirmBag = useCallback(
    async (bagIndices: number[]) => {
      ...
      await storage.updatePlayerGameData(roomId, playerId, {
        bag,
        bagLocked: true,
        hand: remainingHand,
  ...
  const decideSheriff = useCallback(
    async (decision: 'pass' | 'inspect') => {
      ...
      if (!nextTarget) {
        await storage.updateRoomGameState(roomId, { phase: 'end_round' } as Record<string, unknown>);
      }
    },
    [gameState, players, playerId, roomId],
  );
  ...
  const proceedAfterRound = useCallback(async () => {
    ...
    if (isLastRound) {
      ...
      await storage.updateRoomStatus(roomId, 'end');
    } else {
      await startNewRound(gameState);
    }
  }, [gameState, players, roomId, startNewRound, storage]);
  ...
  const advanceMarketPhase = useCallback(async () => {
    ...
    if (allReady) {
      await storage.updateRoomGameState(roomId, { phase: 'load_bag' } as Record<string, unknown>);
    }
  }, [gameState, isHost, merchants, roomId, storage]);
  ...
  const advanceBagPhase = useCallback(async () => {
    ...
    if (allConfirmed) {
      await storage.updateRoomGameState(roomId, { phase: 'declare' } as Record<string, unknown>);
    }
  }, [gameState, isHost, merchants, roomId, storage]);
