...
import type { RoomGameState } from '@core/types/room';
...
export function useShadowHunter(
  ...
  const writeGS = useCallback(
    async (gs: SHGameState) => {
      await storage.updateRoomGameState(room.id, deepClean(gs) as unknown as Partial<RoomGameState>);
    },
    [room.id],
  );
  ...
  const writeMyData = useCallback(
    async (data: Partial<SHPlayerData>) => {
      await storage.updatePlayerGameData(room.id, playerId, data);
    },
    [room.id, playerId],
  );
  ...
  const startGame = useCallback(async () => {
    ...
    for (let i = 0; i < players.length; i++) {
      const char = picked[i];
      await storage.updatePlayerGameData(room.id, players[i].id, {
        characterId: char.id,
        faction: char.faction,
        maxHp: char.hp,
        usedSkill: false,
    ...
    await storage.updateRoomStatus(room.id, 'day');
  ...
  const giveHermitCard = useCallback(async (targetPlayerId: string) => {
    ...
    await storage.updatePlayerGameData(room.id, targetPlayerId, {
  ...
  const resolveHermitCard = useCallback(async (lie = false) => {
    ...
    if (card?.effectId === 'SHOW_CHARACTER_TO_CARD_OWNER') {
      await storage.sendPrivateMessage(
        room.id,
        giverPlayerId,
  ...
  const resetGame = useCallback(async () => {
    if (!isHost) return;
    await storage.clearGameData(room.id);
    await storage.updateRoomStatus(room.id, 'lobby');
    for (const p of players) {
      await storage.updatePlayerGameData(room.id, p.id, {});
    }
