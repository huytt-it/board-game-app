...
import type { ClocktowerGameState } from '../types';
...
export function useVoting(
  ...
  gameState: ClocktowerGameState | undefined,
  alivePlayers: number
): UseVotingReturn {
  ...
  const castNomination = useCallback(
    async (targetId: string | null) => {
      ...
      await storage.updateRoomGameState(roomId, payload as any);
    },
    [roomId, playerId]
  );
  ...
  const castGhostVote = useCallback(
    async (vote: boolean) => {
      ...
      await storage.updatePlayerGameData(roomId, playerId, { hasUsedGhostVote: true });
    },
    [roomId, playerId]
  );
  ...
  const resolveVote = useCallback(
    async (executedRole?: string) => {
      ...
      if (executed) {
        await storage.updatePlayerAlive(roomId, votingTarget, false);
      }
