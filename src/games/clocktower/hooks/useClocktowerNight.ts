...
import type { SubmitActionPayload } from '@core/types/actions';
import { FIRST_NIGHT_ROLES, OTHER_NIGHT_ROLES, ClocktowerRole } from '../types';
...
export function useClocktowerNight(
  ...
  useEffect(() => {
    ...
    const unsub = storage.subscribeToPlayer(roomId, playerId, (player: Player | null) => {
      if (player) {
  ...
  const submitAction = useCallback(
    async (
      ...
      await storage.submitAction(roomId, payload);
