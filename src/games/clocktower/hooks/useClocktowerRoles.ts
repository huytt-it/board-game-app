...
import type { Room, RoomConfig } from '@core/types/room';
import type { Player } from '@core/types/player';
import { ClocktowerRole, ROLE_TEAMS } from '../types';
...
export function useClocktowerRoles(
  ...
  const assignRoles = useCallback(async () => {
    ...
    for (let i = 0; i < gamePlayers.length; i++) {
      ...
      await storage.updatePlayerGameData(roomId, gamePlayers[i].id, {
        role,
        team,
        isPoisoned: false,
        isDrunk,
    ...
    await storage.updateRoomGameState(roomId, { rolesAssigned: true, dayCount: 0 });
  }, [roomId, players, room]);
