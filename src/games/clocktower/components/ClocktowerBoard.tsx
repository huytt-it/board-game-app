...
import { useClocktowerRoles } from '@games/clocktower/hooks/useClocktowerRoles';
import { useVoting } from '@games/clocktower/hooks/useVoting';
import { useStorage } from '@core/services/storage/StorageContext';
import QRCodeDisplay from '@core/components/QRCodeDisplay';
...
import type { GameModuleProps } from '@core/lib/gameRegistry';
import {
  ROLE_ICONS, ROLE_NAMES_VI, ROLE_TEAM_VI, ROLE_TEAMS,
  ClocktowerRole, type ClocktowerTeam, type ClocktowerGameState,
} from '@games/clocktower/types';
...
export default function ClocktowerBoard({ room, players, playerId, isHost }: GameModuleProps) {
  ...
  const gameState = room.gameState as ClocktowerGameState | undefined;
  ...
  } = useVoting(room.id, playerId, gameState, alivePlayers);
  ...
  const handleSlayerUse = useCallback(
    async (targetId: string, targetName: string) => {
      ...
      await storage.updatePlayerGameData(room.id, playerId, { hasUsedAbility: true });
      ...
      <div className="mx-auto max-w-3xl space-y-6 animate-fade-in">
        ...
        <HostDashboard
          ...
          onChangePhase={(phase) => {
            ...
            if (phase === 'day') {
              updateGameState({ dayCount: (gameState?.dayCount || 0) + 1, ...resetVoting });
            } else {
