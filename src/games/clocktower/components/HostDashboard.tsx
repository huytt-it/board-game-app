...
import { useNightActions } from '@games/clocktower/hooks/useNightActions';
import { useVoting } from '@games/clocktower/hooks/useVoting';
import { useGameHistory } from '@core/hooks/useGameHistory';
import { useStorage } from '@core/services/storage/StorageContext';
import type { Player } from '@core/types/player';
import type { GameAction } from '@core/types/actions';
import { ROLE_ICONS, ROLE_NAMES_VI, ROLE_TEAMS, ClocktowerRole, type ClocktowerGameState } from '@games/clocktower/types';
...
export default function HostDashboard({
  ...
  const toggleAlive = async (playerId: string, currentAlive: boolean) => {
    ...
    await storage.updatePlayerAlive(roomId, playerId, !currentAlive);
  ...
  const handleResolve = async (action: GameAction) => {
    ...
    if (actorRole === ClocktowerRole.Poisoner && action.targetId) {
      ...
      if (previousVictim) {
        await storage.updatePlayerGameData(roomId, previousVictim.id, { isPoisoned: false });
        await addEvent({
          type: 'state_change',
          dayCount,
          phase: 'night',
      ...
      await storage.updatePlayerGameData(roomId, action.targetId, { isPoisoned: true });
      await addEvent({
        type: 'state_change',
        dayCount,
        phase: 'night',
    ...
    if (actorRole === ClocktowerRole.Imp && action.targetId === action.playerId) {
      ...
      if (aliveMinions.length > 0) {
        await storage.updateRoomGameState(roomId, {
        ...
        await storage.updatePlayerAlive(roomId, action.playerId, false);
        await addEvent({
          type: 'night_death',
          dayCount,
          phase: 'night',
  ...
  const handleSlayerKill = async () => {
    ...
    const targetRole = targetPlayer?.gameData?.role as ClocktowerRole | undefined;
    await storage.updatePlayerAlive(roomId, slayer.targetId, false);
    await storage.updateRoomGameState(roomId, { pendingSlayerAction: null } as any);
    await addEvent({
      type: 'ability_used',
      dayCount,
      phase: 'day',
  ...
  const handleStarpassConfirm = async (minionId: string) => {
    ...
    await storage.updatePlayerGameData(roomId, minionId, { role: ClocktowerRole.Imp });
    // Kill the old Imp
    await storage.updatePlayerAlive(roomId, starpass.impPlayerId, false);
    ...
    await storage.updateRoomGameState(roomId, { pendingStarpassAction: null } as any);
  };
  ...
  const handleTriggerRavenkeeper = async (ravenkeeperPlayer: Player) => {
    ...
    await storage.updatePlayerGameData(roomId, ravenkeeperPlayer.id, {
  ...
  const handleSetDaySubPhase = async (subPhase: 'discussion' | 'nomination') => {
    ...
    await storage.updateRoomGameState(roomId, { daySubPhase: subPhase, ...extra } as any);
    await addEvent({
      type: 'phase_change',
      dayCount,
      phase: 'day',
  ...
  const handleNewNight = async () => {
    ...
    await storage.updateRoomGameState(roomId, { daySubPhase: 'discussion' } as any);
